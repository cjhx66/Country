...
(function($) {
	...
	$.ligerManagers.Grid.prototype = {
		...
		loadServerData : function(param, clause) {
			...
			var ajaxOptions = {
				...
				error : function(XMLHttpRequest, textStatus, errorThrown) {
					...
					g.currentData[p.record] = g.data[p.record] = 0;
					g.gridloading.hide();
					$(".l-bar-btnload span", g.toolbar).removeClass(
							"l-disabled");
					try {
						if (p.onError)
		...
		collapseleaf : function(rowParm) {
			var po = this.po, g = this, p = this.options;
			var row = $("tbody:first > tr[treelevel=" + rowParm + "]",
					g.gridbody);
		...
		updateCell : function(cell, value, rowParm) {
			...
			var cellContent = po.getCellContent(rowData, rowindex, value,
					column, p.tree, row.attr("treelevel"), rowid);
		...
		addRow : function(rowdata, rowParm, isBefore, parentRow) {
			...
			if (parentRow) {
				...
				parentRowIsOpened = $(".l-grid-tree-link:first", parentRowObj)
						.hasClass("l-grid-tree-link-open");
			}

			var olddatanumber = parentRowData ? parentRowData[p.tree.childrenName].length
					: g.currentData[p.root].length;
			var rowObj = g.getRowObj(rowParm);
			var rowindex = rowObj ? (parseInt($(rowObj).attr("rowindex")) + (isBefore ? 0
					: 1))
					: olddatanumber;
			var rowHTML = po.getHtmlFromData([ rowdata ], p.tree,
					treelevel ? treelevel : 1, parentrowid);
			var row = $(rowHTML);
			po.recordRow(row[0]);
			...
			if (p.dataAction == "local") {
				...
				if (p.sortName == columnName) {
					...
					g.sortedData[p.root].sort(function(data1, data2) {
						return po.compareData(data1, data2, columnName,
								columnType);
					});
				}
				if (p.usePager)
		...
		getRowObj : function(rowParm) {
			...
			if (typeof (rowParm) == "string" || typeof (rowParm) == "number") {
				return $("tbody:first > .l-grid-row[rowid=" + rowParm + "]",
						g.gridbody).get(0);
		...
		isLeaf : function(rowParm) {
			...
			if (!rowObj)
				return;
			return !$("> td > div > .l-grid-tree-space:last", rowObj).hasClass(
					"l-grid-tree-link");
		},
		...
		appendRow : function(rowData, targetRow, nearRow, isBefore) {
			...
			var children = $(targetRowObj).nextAll(
					"tr[parentrowid=" + rowid + "]").get();
			if (!children)
				return;
		...
		upgrade : function(targetRow) {
			...
			$("> td > div > .l-grid-tree-space:last", targetRow).addClass(
		...
		toggle : function(targetRow) {
			...
			var opening = true;
			if (linkbtn.hasClass("l-grid-tree-link-close")) {
				linkbtn.removeClass("l-grid-tree-link-close").addClass(
			...
			while (true) {
				...
				if (treelevel <= level)
					break;
				if (opening) {
					$(".l-grid-tree-link", currentRow).removeClass(
							"l-grid-tree-link-close").addClass(
							"l-grid-tree-link-open");
					currentRow.show();
				} else {
					$(".l-grid-tree-link", currentRow).removeClass(
							"l-grid-tree-link-open").addClass(
	...
	$.ligerAddGrid = function(grid, p) {
		...
		var gridhtmlarr = [];
		gridhtmlarr
				.push("        <div class='l-grid-panel-header'><span class='l-panel-header-text'></span></div>");
		gridhtmlarr
		...
		gridhtmlarr.push("                <div class='l-bar-separator'></div>");
		gridhtmlarr
				.push("                <div class='l-bar-group'><span class='pcontrol'> <input type='text' size='4' value='1' style='width:30px' maxlength='3' /> / <span></span></span></div>");
		gridhtmlarr.push("                <div class='l-bar-separator'></div>");
		gridhtmlarr.push("                <div class='l-bar-group'>");
		gridhtmlarr
		...
		var po = {
						...
						.each(
								function(i, item) {
									...
									if (!colwidth) {
										...
										colwidth = parseInt((g.gridbody.width() - lwidth)
												/ g.columns.length);
									}
			...
			initBuildPopup : function() {
				$("tr:last .l-grid-hd-cell", g.gridheader)
						.each(
								function(i, td) {
									if ($(this).hasClass(
											"l-grid-hd-cell-detail"))
										return;
									var isAllowHide = $(this).attr(
											"isAllowHide");
									if (isAllowHide != undefined
											&& isAllowHide.toLowerCase() == "false")
										return;
									var chk = 'checked="checked"';
									var columnindex = $(this).attr(
											"columnindex");
									var columnname = $(this).attr("columnname");
									if (!columnindex || !columnname)
										return;
									var header = $(".l-grid-hd-cell-text", this)
											.html();
			...
			initEven : function() {
				...
				$(".l-grid-hd-cell-text", g.gridheader)
						.click(
								function(e) {
									...
									$(".l-grid-hd-cell-sort", row.siblings())
											.remove();
									return false;
								});
							...
							.mousemove(
									function(e) {
										if (g.colresize)
											return; // 如果正在调整列宽
										var posLeft = e.pageX
												- $(grid).offset().left; // 当前鼠标位置
										var currentLeft = 0;
										for ( var i = 0; i < g.headers.length; i++) {
											var hide = false;
											if (g.headers[i].columnindex != undefined) {
												hide = g.columns[g.headers[i].columnindex].hide ? true
														: false;

											}
											if (!hide && g.headers[i].width)
						...
						.click(
								function(e) {
									...
									if (obj.tagName.toLowerCase() == "span"
											&& $(obj)
													.hasClass(
															"l-grid-row-cell-detailbtn")) {
										var row = $(obj).parent().parent()
												.parent();
										// 确保不是在内嵌表格点击的
										if (row.parent().parent()[0] != $(
												"table:first", g.gridbody)[0])
											return;
										var rowindex = parseInt($(row).attr(
												"rowindex"));
										var rowid = $(row).attr("rowid");
										var item = g.getRow(rowid);
										if ($(obj).hasClass("l-open")) {
											row.next(".l-grid-detailpanel")
													.hide();
											$(obj).removeClass("l-open");
										} else {
											var nextrow = row
													.next(".l-grid-detailpanel");
											if (nextrow.length > 0) {
												nextrow.show();
												$(obj).addClass("l-open");
												return;
											...
											var detailRowInner = $("div:first",
													detailRow);
											detailRowInner.parent()
													.attr("colSpan",
															g.headers.length);
											row.after(detailRow);
											if (p.detail.onShowDetail) {
												p.detail.onShowDetail(item,
														detailRowInner[0]);
												detailRowInner.show();
											} else if (p.detail.render) {
												detailRowInner.append(p.detail
														.render());
												detailRowInner.show();
											}
											$(obj).addClass("l-open");
									...
									if (obj.tagName.toLowerCase() == "div"
										...
										if (p.onBeforeCheckAllRow) {
											if (p.onBeforeCheckAllRow(!uncheck,
													grid) == false)
												return false;
										}
										if (uncheck) {
											row.removeClass("l-checked");
											$("tbody:first > tr.l-grid-row",
													g.gridbody).removeClass(
													"l-checked");
										} else {
											row.addClass("l-checked");
											$("tbody:first > tr.l-grid-row",
			...
			showData : function(data) {
				...
				// 加载中
				$('.l-bar-btnloading:first', g.toolbar).removeClass(
						'l-bar-btnloading');
				if (!data || !data[p.root])
					return;
				...
				$(".l-bar-btnload:first span", g.toolbar).removeClass(
						"l-disabled");
				po.clearGrid();
					...
					$(groupsdata)
							.each(
									function(i, item) {
										...
										if (p.groupRender)
											gridhtmlarr.push(p.groupRender(
													groups[i],
				...
				po.onResize();
				// 分组 - 事件
				$("tbody:first > .l-grid-grouprow", g.gridbody)
						.each(
								function() {
									var grouprow = $(this);
									$(".l-grid-group-togglebtn", grouprow)
											.click(
													function() {
														var opening = true;
														if ($(this)
			...
			onClick : function(e) {
				...
				if (p.allowHideColumn) {
					if (tagName == 'html'
							...
							|| $(obj).hasClass("l-grid-grouprow-cell")
							|| $(obj).hasClass("l-grid-totalsummary-cell")
							|| $(obj)
									.hasClass("l-grid-totalsummary-cell-inner")) {
						g.popup.hide();
					}
				}
			},
				...
				$(dataArray)
						.each(
								function(i, item) {
									if (!item)
										return;
									if (!p.usePager && i == rowlenth - 1
											&& !g.isTotalSummary())
										gridhtmlarr
												.push('<tr class="l-grid-row l-grid-row-last');
									else
										gridhtmlarr
												.push('<tr class="l-grid-row');
									if (tree) {
										gridhtmlarr.push(' l-grid-treerow');
									}
									if (p.checkbox && p.isChecked
											&& p.isChecked(item)) {
									...
									g.recordNumber++;
									if (parentrowid != undefined)
										gridhtmlarr.push(" parentrowid= "
												+ parentrowid);
									gridhtmlarr.push(' rowindex="' + i + '">');
									$(g.headers)
											.each(
													function(headerCellIndex,
															headerInfor) {
														// 如果是复选框(系统列)
														if (this.ischeckbox) {
															gridhtmlarr
																	.push('<td class="l-grid-row-cell l-grid-row-cell-checkbox" style="width:'
																			+ this.width
																			+ 'px"><div class="l-grid-hd-cell-inner"><span class="l-grid-row-cell-btn-checkbox"></span></div></td>');
															return;
														}
														// 如果是明细列(系统列)
														...
														if (column
																&& column.type == "date") {
															var date = p
																	.renderDate(item[this.columnname]);
					...
					$(rowobj).bind(
							...
							function(e) {
								...
								if (p.onRClickToSelect)
									$(this).addClass("l-selected").siblings(
						...
						.click(
								function(e) {
									...
									if ($(this).hasClass("l-selected")) {
										...
										if (p.onUnSelectRow) {
											p.onUnSelectRow(g.getRow(rowid),
													index, this);
										}
									} else {
										$(this)
			...
			applyEditor : function(obj) {
				...
				if (column.editor.type == 'date') {
					...
					if (column.editor.p)
						options = $
								.expend(
										{},
										typeof (column.editor.p) == 'function' ? column.editor
												.p(rowdata, rowindex,
														currentdata, column)
					...
					if (column.editor.p) {
						var tmp = typeof (column.editor.p) == 'function' ? column.editor
								.p(rowdata, rowindex, currentdata, column)
								: column.editor.p;
						options = $.extend({}, options, tmp);
					}
					$inputText.ligerComboBox(options);
				} else if (column.editor.type == 'int'
						|| column.editor.type == 'float'
						|| column.editor.type == 'spinner') {
					var $inputText = $("<input type='text'/>");
					g.grideditor.append($inputText);
					$inputText.attr({
						style : 'border:#6E90BE'
				...
				} else if (column.editor.type == 'string'
				...
				} else if (column.editor.type == 'chk'
					...
					$input.change(function() {
						if (column.editor.onChange)
							column.editor.onChange(rowcell, this.checked);
						editParm.value = this.checked ? 1 : 0;
						if (po.checkEditAndUpdateCell(editParm)) {
							if (column.editor.onChanged)
								column.editor.onChanged(rowcell, this.checked);
						}
					});
				}
				g.grideditor.show();
			...
			getCurrentPageData : function(jsonObj) {
				...
				data[p.record] = jsonObj[p.root].length ? jsonObj[p.root].length
						: 0;
				if (!p.newPage)
					p.newPage = 1;
			...
			compareData : function(data1, data2, columnName, columnType) {
				...
				switch (columnType) {
				case "int":
					return parseInt(data1[columnName]) < parseInt(data2[columnName]) ? -1
							: parseInt(data1[columnName]) > parseInt(data2[columnName]) ? 1
									: 0;
				case "float":
					return parseFloat(data1[columnName]) < parseFloat(data2[columnName]) ? -1
							: parseFloat(data1[columnName]) > parseFloat(data2[columnName]) ? 1
									: 0;
				case "string":
					return data1[columnName].localeCompare(data2[columnName]);
				case "date":
					return data1[columnName] < data2[columnName] ? -1
			...
			getTotalSummaryHtml : function(data, classCssName) {
				...
				$(g.headers)
						.each(
								function(headerCellIndex, headerInfor) {
									// 如果是复选框(系统列)
									if (this.ischeckbox) {
										totalsummaryArr
									...
									if (columnname) {
										totalsummaryArr.push('columnname="'
												+ columnname + '" ');
									}
									totalsummaryArr.push('columnindex="'
											+ columnindex + '" ');
									totalsummaryArr
									...
									var column = g.columns[columnindex];
									if (column.align)
										totalsummaryArr.push(' textAlign="'
												+ column.align + '"');
									totalsummaryArr.push('>');
									...
									if (column.totalSummary) {
										...
										if (column.totalSummary.render) {
											...
											if (isExist('total'))
												totalsummaryArr
														.push("<div style='text-align:right;'>总计：</div>");
											if (isExist('sum_money'))
												// totalsummaryArr.push("<div>Sum="
												// + sum.toFixed(2) + "</div>");
												totalsummaryArr
														.push("<div style='text-align:right;'>￥"
																+ toMoney(sum
																		.toFixed(0))
																+ "</div>");
										}
			...
			buildPager : function() {
				...
				if (p.page == 1) {
					...
					$(".l-bar-btnfirst span", g.toolbar).removeClass(
							"l-disabled");
					$(".l-bar-btnprev span", g.toolbar).removeClass(
							"l-disabled");
				}
				if (p.page == p.pageCount) {
					$(".l-bar-btnlast span", g.toolbar).addClass("l-disabled");
					...
					$(".l-bar-btnlast span", g.toolbar).removeClass(
							"l-disabled");
					$(".l-bar-btnnext span", g.toolbar).removeClass(
							"l-disabled");
				}
			},
			onResize : function() {
			...
			dragEnd : function(e) {
				if (g.colresize) {
					...
					$("div:first", g.gridheader).width(g.gridtablewidth + 40);
					$("div:first", g.gridbody).width(g.gridtablewidth);
					$('td[columnindex=' + columnindex + ']', g.gridheader).css(
							'width', newwidth);
					if (g.recordNumber > 0) {
						$('td[columnindex=' + columnindex + ']', g.totalRows)
								.add(g.columnCells[columnindex]).each(
										function() {
											$(this).css('width', newwidth);
											$("div:first", this).css('width',
													newwidth - 8);
										});
	...
	var ligerGridSetParms = function(options, fixedP) {
		var p = $.extend({}, $.ligerDefaults.Grid, $.ligerDefaults.GridString,
				options || {});
		if (p.url && p.data) {
			p.dataType = "local";
